...
**评审类请求必须先判断评审对象（对象优先于关键词）**：

以下表达应路由到 **Review Agent**（评审对象 = 产品方案）：
- "评审需求"、"检查 PRD"、"看看设计是否合理"
- "业务逻辑有没有问题"、"审核方案"、"流程是否完整"
- 对话上文是产品设计，用户说"帮我看看有没有问题"

以下表达应路由到 **Frontend Agent → Frontend Review**（评审对象 = 前端代码）：
- "评审页面实现"、"检查前端代码"、"对刚刚实现的代码做评审校验"
- "看看页面有没有遗漏"、"检查代码是否符合需求"、"验收前端"
- "按钮能不能用"、"交互是否完整"、"是否影响已有代码"
- 对话上文是前端实现，用户说"帮我检查一下"

以下表达应路由到 **Frontend Agent**（非评审，实现类）：
...
"看看PRD和页面实现是否一致"
→ 评审对象包含产品方案 + 前端代码
→ 阶段 1：Review Agent → prd-review（先检查产品设计合理性）
→ 阶段 2：Frontend Agent → frontend-implement Frontend Review（再检查代码是否实现需求）
```

**前端实现完成后的默认行为（v4.3）**：

```text
frontend-implement 完成代码实现
    ↓
（不要直接结束）
    ↓
自动进入 Frontend Review 阶段
    ↓
输出实现检查报告
    ↓
发现问题 → 修复 → 重新自检 → 通过后结束
